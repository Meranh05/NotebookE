...
worker-start:
	...
	uv run --env-file .env surreal-commands-worker start --import-modules commands --max-tasks "$${OPEN_NOTEBOOK_WORKER_MAX_TASKS:-5}"
...
# === Service Management ===
start-all:
	@echo "🚀 Starting NotebookE (Database + API + Worker + Frontend)..."
...
stop-all:
	@echo "🛑 Stopping all NotebookE services..."
