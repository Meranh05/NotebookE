...
from api.routers import video_test as video_test_router
...
app = FastAPI(
    title="NotebookE API",
    description="API for NotebookE - Research Assistant",
    lifespan=lifespan,
)
...
app.include_router(video_test_router.router, prefix="/api", tags=["video-test"])


@app.get("/")
async def root():
    return {"message": "NotebookE API is running"}
