# Backend Rules (api/ + notebooke/ + commands/ + prompts/)
...
## AI / model provisioning (`notebooke/ai/`)
...
## Database (`notebooke/database/`)

- New migration = new file `notebooke/database/migrations/N.surrealql` (+ `N_down.surrealql`) **and** an edit to `AsyncMigrationManager` — migrations are hard-coded, not auto-discovered. They run automatically on API startup.
